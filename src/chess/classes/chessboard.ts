...
export class Chessboard<T extends BasePiece> {
    ...
    constructor(blacks?: Map<number, T>, whites?: Map<number, T>, capturedBlacks?: T[], capturedWhites?: T[]) {
    ...
    public clone(): Chessboard<T> {
        return new Chessboard<T>(this.cloneMap(this.blacks), this.cloneMap(this.whites), this.cloneArray(this.capturedBlacks), this.cloneArray(this.capturedWhites));
    }
    ...
    private cloneMap(map: Map<number, T>): Map<number, T> {
        return new Map([...map.entries()].map(([key, value]) => [key, value.clone() as T]));
    }
